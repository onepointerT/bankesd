...
hash_t* bankesd_ciphers_sha( const char* msg, const unsigned short bitlength, const unsigned int rounds ) {
    ...
    _blockbuf_t* blkbuf_msg512 = _bankesd_blockbuf_init( msg_length / 512, 512 );
    unsigned long startpos_msg = 0;
    while ( blkbuf_msg512->bufferindex < blkbuf_msg512->buffercount ) {
        _bankesd_blockbuf_strcpy_n( blkbuf_msg512, msg, startpos_msg, 0, 512 );
        _bankesd_blockbuf_switchbuf( blkbuf_msg512, blkbuf_msg512->bufferindex + 1 );
        startpos_msg += 512;
    }


    _blockbuf_t* blkbuf_word = _bankesd_blockbuf_init( 80, 32 );

    hash_t* sha = bankesd_ciphers_init_hash( bitlength );
    ...
    for ( unsigned long i = 0; i < blkbuf_msg512->buffercount; i++ ) {
        ...
        for ( unsigned int ic = 0; ic < 16 && isha < bitlength; ic++ ) { 
            ...
            _bankesd_blockbuf_switchbuf( blkbuf_word, wbuf );
            _bankesd_blockbuf_strcpy( blkbuf_word, word, 0, 0 );
            ++wbuf;
